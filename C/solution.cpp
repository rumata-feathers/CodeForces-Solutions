...
struct AVL_tree{
    ...
    int height = 0;
    int balance = 0;
    long long amount_less = 0;
    long long amount_more = 0;
...
void fix_vertex(AVL_tree* vertex){
    ...
    if (vertex->left_son != nullptr) {
        left_height = vertex->left_son->height;
        vertex->amount_less = vertex->left_son->amount_less + vertex->left_son->amount_more + 1;
    }else{
        vertex->amount_less = 0;
    }
    if (vertex->right_son != nullptr) {
        right_height = vertex->right_son->height;
        vertex->amount_more = vertex->right_son->amount_less + vertex->right_son->amount_more + 1;
    }else{
        vertex->amount_more = 0;
    }
...
AVL_tree* get_kth(AVL_tree* vertex, long long k){
    if (vertex->amount_less + 1 == k) return vertex;
    else if (vertex->amount_less + 1 > k) return get_kth(vertex->left_son, k);
    else return get_kth(vertex->right_son, k - vertex->amount_less - 1);
}
int main() {
    ...
    for (int i = 0; i < quests_amount; ++i) {
        std::cin >> init_command >> num;
        if (init_command == "1" || init_command == "+1"){
            root = insert(root, num);
        }
        if (init_command == "0") {
            std::cout << get_kth(root, root->amount_less + root->amount_more + 1 - num + 1)->value << '\n';
        }
        if (init_command == "-1"){
